...
class KitData:
    ...
    ORDER By
      p.product_number,
      p2.product_number""".format(kit_id)
        self.pg.establish_connection()
        kit_compnt_std, headers = self.pg.execute(query)
...
WHERE
  ...
  AND s9.kit_product_number = '{}'
ORDER BY
  s4.percent_invalid DESC,
  s9.kit_product_number,
  s9.serial_number,
  s9.component_product_number""".format(kit_id)
        self.pg.establish_connection()
        kit_report, headers = self.pg.execute(query)
        self.pg.close_connection()
